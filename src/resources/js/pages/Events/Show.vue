...
<template>
    <DefaultLayout>
        <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
            <div class="mb-6">
                <Link href="/eventos" class="inline-flex items-center text-sm text-gray-500 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2">
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <div class="mb-6 flex flex-col gap-3 text-gray-600 sm:flex-row sm:gap-6">
                        <div class="flex items-center">
                            ...
                            <svg class="mr-2 h-5 w-5 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path
                                    stroke-linecap="round"
                                    stroke-linejoin="round"
                                    stroke-width="2"
                            ...
                            <span>{{ formatDateRange(props.event.startDate, props.event.endDate) }}</span>
                        </div>
                    ...
                    <div v-if="props.event.memberPrice !== null" class="mb-6 rounded-lg bg-amber-50 p-4">
                        <h3 class="mb-2 font-semibold text-gray-900">{{ t('events.pricing') }}</h3>
                        <div class="flex flex-col gap-1 sm:flex-row sm:gap-6">
                            ...
                            <p v-if="props.event.nonMemberPrice !== null" class="text-gray-700">
                                <span class="font-medium">{{ t('events.nonMemberPrice') }}:</span>
                                {{ formatPrice(props.event.nonMemberPrice) }}
                            </p>
                        </div>
                    </div>
                    <div v-else class="mb-6 rounded-lg bg-green-50 p-4">
                        <p class="font-medium text-green-800">{{ t('events.free') }}</p>
                    </div>

                    <div class="prose prose-amber max-w-none">
                        <p class="whitespace-pre-line text-gray-700">{{ props.event.description }}</p>
                    </div>
